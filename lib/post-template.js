...
const getFrontMatter = (properties) => {
    ...
    if (properties.summary) {
        properties.excerpt = properties.summary;
    ...
    // Since mp-syndicate-to is not available, use channel as workaround
    if (properties.channel) {
        // Remove default
        properties.channel = properties.channel.filter((channel) => channel !== "default");
        if (properties.channel.length > 0) {
            properties.syndicate_to = properties.channel;
        }
